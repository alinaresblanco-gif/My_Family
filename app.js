...
const SHEETS_API_URL = 'https://script.google.com/macros/s/AKfycbxUYmwpwEk-4pN0gu4br78LaxHIdRUozk1W1n0nB1lRIWuo-vSSICSDpJisyMl7VWX6/exec';
...
function buildCalendar() {
  ...
  calendar.innerHTML = headings.map(day => `<div class="cal-head">${day}</div>`).join('') + Array.from({ length: totalCells }, (_, index) => {
    ...
    const rowStart = getWeekStart(cellDate);
    const rowStartKey = dateKey(rowStart);
    const rowEndDate = new Date(rowStart);
    rowEndDate.setDate(rowEndDate.getDate() + 6);
